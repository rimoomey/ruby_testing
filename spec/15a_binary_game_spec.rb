...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    subject(:verify_input_game) { described_class.new(1, 20) }
    # NOTE: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        valid_input = '15'.to_i
        expect(verify_input_game.verify_input(1, 20, valid_input)).to eq(15)
      end
    end

    context 'when given invalid input as argument' do
      it 'returns nil' do
        invalid_input = 'b'.to_i
        expect(verify_input_game.verify_input(1, 20, invalid_input)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:display_game) { described_class.new(1, 100) }
    let(:search_display) { instance_double(BinarySearch) }
    ...
    it 'increases guess_count by one' do
      expect { display_game.display_turn_order(search_display) }.to change { display_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display).to receive(:make_guess)
      display_game.display_turn_order(search_display)
    end
